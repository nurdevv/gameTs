...
export const ConnectWalletModal: FunctionComponent<{ isOpen: boolean; onClose: () => void }> = ({isOpen, onClose}) => {
    ...
    return <><Modal isOpen={isOpen} onClose={onClose}>
                ...
                </Center>
                {
                    !!walletsList && <Flex gap="2" flexWrap="wrap">{
                        walletsList.map(walletInfo =>
                            <Button onClick={() => onWalletClick(walletInfo)}
                                    leftIcon={<Image w="16px" h="16px" src={walletInfo.imageUrl}/>} key={walletInfo.name}>
                                {walletInfo.name}
                            </Button>
                        )
                    }
        ...
        <QRCodeModal isOpen={!!selectedWalletInfo} onClose={() => setSelectedWalletInfo(null)}
